...
public class ParticleEarlyKill : MonoBehaviour
{
    ...
    void Update ()
    {
        ...
        for (int i = 0; i < iParticleCount; i++)
        {
            ...
            if (fTargetLifeTime > _parParticleList[i].lifetime)
            {

                // Debug.Log("killing Particle");

                //calc max alpha
                float fMaxAlpha = 1 - (_fParticleFadeOutRate * (fTargetLifeTime - _parParticleList[i].lifetime));

                //check if particle alpha is 0
                if (fMaxAlpha < 0)
                {
                    //kill particle
                    _parParticleList[i].lifetime = -1;
